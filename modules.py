...
# Step 1: Auto-generate a password
pwl = random.randint(8, 15)
auto_password = "".join(random.choices(all_chars, k=pwl))
print(f"\n Generated Password: {auto_password}")

# Step 2: Ask if the user is okay with the generated password
user_choice = input("Are you okay with this password? (yes/no): ").lower()

if user_choice == "yes":
    print("Great! Your password is ready to use. Exiting program...")
else:
    # Step 3: Ask the user to input their own password
    ...
    if length < 8 or not (has_number or has_upper or has_symbol):
        ...
        print(f"Your password '{user_password}' is acceptable.")
